...
# --- FUNÇÃO PARA GERAR A FICHA (Sabor Ultra Gigante) ---
def gerar_ficha_imagem(sabor, id_venda, pagto):
    ...
    if os.path.exists(logo_file):
        try:
            ...
            largura_logo = 90 # Logo bem pequena no topo
    ...
    try:
        # Aumentamos a fonte para 110 e usamos Negrito
        f_sabor = ImageFont.truetype("arialbd.ttf", 110) 
        f_info = ImageFont.truetype("arialbd.ttf", 26)
        f_pequena = ImageFont.truetype("arial.ttf", 16)
    except:
        f_sabor = f_info = f_pequena = ImageFont.load_default()

    d.line([(5, y), (295, y)], fill=(0,0,0), width=5)
    y += 100 # Espaço para o texto ultra gigante
    
    # SABOR CENTRALIZADO E ENORME
    ...
    y += 25
    d.text((150, y), "Válido apenas na data de emissão", fill=(0,0,0), font=f_pequena, anchor="ms")
    y += 40
...
if st.session_state.caixa_inicial is None:
    ...
    if st.button("CONFIGURAR CARDÁPIO", use_container_width=True):
        try:
            st.session_state.caixa_inicial = float(v_ini.replace(',','.')) if v_ini else 0.0
            st.rerun()
        except: st.error("Insira um valor numérico.")
    st.stop()

if not st.session_state.configurado:
    st.title("⚙️ Cardápio")
    sab_op = ["Pilsen", "IPA", "Vinho", "Weiss", "Black", "Água", "Refrigerante"]
    selec = st.multiselect("Sabores de Hoje:", sab_op, default=["Pilsen", "IPA"])
    temp_c = {}
    for s in selec:
        p = st.text_input(f"Preço {s}:", value="", placeholder="15.00", key=f"p_{s}")
        try: temp_c[s] = float(p.replace(',','.')) if p else 0.0
        except: temp_c[s] = 0.0
    if st.button("INICIAR VENDAS", use_container_width=True, type="primary"):
    ...
    with cc:
        ...
        if not st.session_state.carrinho: st.info("Carrinho vazio")
            ...
            if st.session_state.show_dinheiro:
                rec = st.text_input("Recebido:", value="", key=f"r_{len(st.session_state.vendas)}")
                if rec:
                    try:
...
with t2:
    ...
    if not st.session_state.vendas: st.info("Sem vendas.")
        ...
        if remover and st.button(f"ESTORNAR {len(remover)} SELECIONADOS", type="primary"):
            for r in remover: st.session_state.vendas.remove(r)
            st.session_state.sel_todos_estorno = False; st.success("Estorno concluído!"); st.rerun()
...
with t3:
    ...
    if st.session_state.vendas:
        ...
        # AQUI ESTAVA O ERRO DA LINHA 205 (CORRIGIDO ABAIXO)
        c4.metric("DINHEIRO", f"R$ {(st.session_state.caixa_inicial + v_din):.2f}")
        
        st.divider()
        st.write("#### Total por Sabor")
        st.table(df_f.groupby('Sabor').size().reset_index(name='Quantidade'))
    
    if st.button("FECHAR EVENTO (LIMPAR TUDO)"):
